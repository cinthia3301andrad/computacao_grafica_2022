...
from intersecoes import IntersecaoCilindro,  IntersecaoEsfera, IntersecaoPlano, IntersecaoCone, IntersecaoCubo
...
def Intersecao_objeto_proximo(Po, D, cena, shadowcheck, tam_pf_pi = math.inf ):
    ...
    for objeto in cena['objetos']:
        ...
        if(objeto['tipo'] == 'plano'):
            ...
            P = Calcula_ponto_intersecao(Po, t1 , D) 
...
def DecideCor(Po, cena, canvas, D, P_F, x, y):
    ...
    if(objeto_encontrado != None and objeto_encontrado['tipo'] == 'esfera'):
        ...
        if (shadowcheck == 1):
            [s, _, _]   = Intersecao_objeto_proximo(P, L, cena, shadowcheck, tam_pf_pi)
        else:
            s = tam_pf_pi
    ...
    if(objeto_encontrado != None and objeto_encontrado['tipo'] == 'cubo'):
            ...
            L                 = Calc_L(P_F, P)
            v_vetor           = normalizaVetor(Subtracao_vetores(Po, P))
            N                 =  normalizaVetor(objeto_encontrado['normal'])
            r_vetor_refletido = normalizaVetor(Calcula_vetor_refletido(L, N))
